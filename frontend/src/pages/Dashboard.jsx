...
export default function Dashboard() {
    ...
    // Główny UseEffect - ładuje profil, wydarzenia i powiadomienia
    useEffect(() => {
        ...
        // --- POBIERANIE WYDARZEŃ I POWIADOMIEŃ ---
                        ...
                        {isUserMenuOpen && (
                            <div className="absolute right-0 mt-4 w-56 bg-[#0f0f0f] border border-white/10 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden py-3 animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="px-6 py-4 border-b border-white/5 mb-2">
                                    ...
                                    <p className="text-xs font-black italic text-green-500">{username}</p>
                                </div>
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                ...
                {!loading && !error && events.length === 0 && (
                    <div className="text-center mt-20 p-20 bg-[#0f0f0f] rounded-[3rem] border border-white/5 shadow-2xl">
                        ...
                        <button onClick={() => setIsModalOpen(true)} className="bg-white text-black font-black uppercase text-xs tracking-widest px-10 py-5 rounded-2xl hover:bg-green-500 transition-all">
                            ...
                            <div key={event.id} onClick={() => navigate(`/events/${event.id}`)} className="group bg-[#0f0f0f] p-8 rounded-[2.5rem] border border-white/5 hover:border-green-500/30 hover:bg-[#151515] transition-all cursor-pointer relative overflow-hidden shadow-2xl">
                                <div className="relative z-10">
                                    ...
                                    <div className="pt-6 border-t border-white/5 flex justify-between items-center">
                                        <button onClick={(e) => { e.stopPropagation(); navigate(`/events/${event.id}/map`); }} className="text-[9px] font-black uppercase tracking-widest bg-black hover:bg-green-600 px-4 py-2 rounded-xl transition-all border border-white/10">
                                            Mapa 📍
                                        </button>
                    ...
                    <div className="bg-[#0f0f0f] rounded-[3rem] p-10 w-full max-w-md border border-white/10 shadow-2xl">
                        ...
                        <form onSubmit={handleCreateEvent} className="space-y-6">
                            ...
                            <div className="flex gap-4 pt-4">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-white/5 hover:bg-white/10 text-gray-400 font-black uppercase text-[10px] tracking-widest py-5 rounded-2xl transition-all">
                                    Anuluj
                                </button>
                                <button type="submit" className="flex-1 bg-green-600 hover:bg-green-500 text-white font-black uppercase text-[10px] tracking-widest py-5 rounded-2xl shadow-xl shadow-green-900/20 transition-all">
                                    Zapisz
                                </button>
                            </div>
                        </form>
